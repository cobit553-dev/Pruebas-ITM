            ...
            <div style="background:#ffffff; border:1px solid #e5e7eb; border-radius:14px; padding:24px;">
                ...
                <form method="POST" action="{{ route('admin.inscripciones.store') }}" style="display:grid; grid-template-columns:1fr 1fr auto; gap:12px; align-items:end;">
                    ...
                    <div>
                        ...
                        <select name="alumno_id" required style="width:100%; padding:10px 14px; font-size:13px; border:1px solid #e5e7eb; border-radius:10px; background:#f9fafb; color:#111827; outline:none;">
                            <option value="">Seleccionar alumno...</option>
                            @foreach($alumnos as $alumno)
                            <option value="{{ $alumno->id }}">{{ $alumno->nombre }} {{ $alumno->apellido }} — {{ $alumno->codigo }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <button type="submit" style="padding:10px 20px; background:#111827; color:#fff; border:none; border-radius:10px; font-size:13px; font-weight:600; cursor:pointer; white-space:nowrap;"
            ...
            <div style="background:#ffffff; border:1px solid #e5e7eb; border-radius:14px; overflow:hidden;">
                ...
                <table style="width:100%; border-collapse:collapse;">
                    ...
                    <tbody>
                        ...
                        @empty
                        <tr>
                            <td colspan="7" style="padding:40px; text-align:center; color:#9ca3af; font-size:13px;">No hay inscripciones registradas.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
